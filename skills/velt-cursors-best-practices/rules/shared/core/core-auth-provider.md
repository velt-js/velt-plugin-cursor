...
tags: auth, authProvider, VeltProvider, identity, authentication, cursors
---
...
Using deprecated authentication methods can cause silent session expiry, broken cursor state when tokens expire, and no automatic recovery from transient network failures. The `authProvider` pattern centralizes auth and keeps Velt in sync with your app's auth lifecycle. Cursors will not render for unauthenticated users.
